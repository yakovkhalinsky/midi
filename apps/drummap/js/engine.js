...
class DrumMap {
  ...
  modulate(param, ch) {
    return con(param + this.cv[ch], 0, DRUM_MAX_VAL);
  }
  ...
  onClock(t) {
    ...
    for (let ch = 0; ch < 2; ch++) {
      ...
      level = con(level + this.randomness[part], 0, DRUM_MAX_VAL);
  ...
  update(now, dt) {
    const dTicks = dt * TICKS_PER_SECOND;
    for (let ch = 0; ch < 2; ch++) {
    ...
    if (now - this.lastClock > DRUM_AUTO_RESET_TICKS / TICKS_PER_SECOND && this.step !== 0) {
      this.step = 0;
    }
  }
