 ...
 * Source: ../melogen/js/synth.js — regenerate (re-copy + re-prefix) if Melogen synth changes.
 * MelogenVoice kept unprefixed (no TB-3PO collision); midiToFreq → MG_midiToFreq. */
'use strict';
/*
...
class MelogenVoice {
  ...
  noteOn(id, pitch, velocity, when) {
    ...
    const targetFreq = MG_midiToFreq(pitch);
    const canGlide = this.lastPitch != null && p.slideTau > 0.0005;
    if (canGlide) {
      osc.frequency.setValueAtTime(MG_midiToFreq(this.lastPitch), t);
    ...
    this.active.set(id, { osc, filter, gain, pitch, ampLevel });
  }
  ...
  noteOff(id, when) {
    ...
    try {
      ...
      // Prefer stored ampLevel when scheduling ahead (rack lookahead), else live value
      const level = (t > ctx.currentTime + 0.002 && v.ampLevel != null)
        ? v.ampLevel : Math.max(0.0001, v.gain.gain.value);
      v.gain.gain.setValueAtTime(Math.max(0.0001, level), t);
