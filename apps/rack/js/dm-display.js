/* AUTO-GENERATED for the rack: copy of ../drummap/js/display.js with the globals
...
'use strict';
/*
 * 64x64 one-bit DM_OLED renderer (weegfx-style primitives) plus a port of
...
class DM_OLED {
  ...
  print(x, y, str) {
    ...
    const s = String(str);
    for (const ch of s) {
      const g = DM_fontGlyph(ch.charCodeAt(0));
...
function drawSlider(ol, x, y, len, value, isCursor) {
  ...
  const w = DM_proportion(value, DRUM_MAX_VAL, len - 1);
